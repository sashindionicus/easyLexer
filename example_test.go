...
func Example() {
	...
	lexer := easylexer.NewLexer(strings.NewReader(input))
...
func Example_addOriginalTokenType() {
	const (
		SUBSITUATION easylexer.TokenID = iota
		NEWLINE
	)
	...
	lexer.Whitespace = easylexer.NewPatternTokenType(-1, []string{"\t", " "})
	// lexer.Whitespace = easylexer.NewRegexpTokenType(-1, `[\t ]`)  // same mean above

	lexer.TokenTypes = append([]easylexer.TokenType{
		easylexer.NewPatternTokenType(SUBSITUATION, []string{"="}),
		easylexer.NewRegexpTokenType(NEWLINE, `^[\n\r]+`),
	}, lexer.TokenTypes...)
